...
public class CounterView extends VBox implements  Observer  {
    ...
    private User user;
    // Pour mieux l'utiliser dans la méthode update => Observers.
    private Label label_text_number;
    private VBox vbox_panel;
    public CounterView(BorderPane root_border_panne, CounterModel counter_model,User user){
    ...
    public Label get_label_text_number(){return this.label_text_number;}

    public VBox get_Vbox_panel(){return this.vbox_panel;}
     /*
    STRUCTURE GENERALE :
    ...
    // On crée la scène VBox et on lui ajoute tous les éléments nécessaires pour compteur.
    public VBox get_vbox_panel(){
        // Début de la VBox.
        // ------------------------------------------------
            vbox_panel = new VBox(5); // Impose un espace de 5 px entre chaque étage.
            ...
            // ------------------------------------------------------
                ...
                this.get_user().set_user_name(textField_name_user.getText()); // après l'exécution de cette méthode, il excutera la méthode update.
                /*
            ...
            // ---------------------------------------------------------------------------------
                ...
                // -----------------------------------------------------------
                    ...
                    // Je le mets ici en prévison de les utiliser dans les event de btn_less et btn_add.
                    this.label_text_number = new Label();
    ...
    public void update(java.util.Observable o, Object arg) {
        // Si o est bien une instance de la classe Modèle dont je veux observer le changement.
        if(o instanceof CounterModel ){
            /*
            Si cette variable (précisément) de l'instance de cette classe o est bien celle dont le dont le changement à été observé.
            Le scénario du faux positif :
            (Imaginons que ton modèle possède deux variables : nbr (qui vaut 10) et score (qui vaut 0).)

                1. Dans ton programme, le score change. Il passe à 100. (La variable nbr n'a pas bougé, elle vaut toujours 10).
                2. Le modèle crie notifyObservers().
                3. Ta méthode update(o, arg) se déclenche.
                4. Ton code arrive sur ta condition :
                    if ( ((CounterModel) o).get_nbr() == this.counter_model.get_nbr() ){}
                5. Java lit o.get_nbr(). Il trouve 10.
                6. Java lit this.counter_model.get_nbr(). Il trouve 10.
                7. Est-ce que 10 == 10 ? Oui.
            Résultat : Ton if est validé ! Ton code va exécuter l'intérieur du if en croyant que c'est nbr qui a changé... alors que c'était le score !
            Pourquoi ça fait ça ?
                Parce que this.counter_model ne garde pas une photo de l'ancien état.
                Il regarde le modèle en temps réel, exactement comme o.
                Si tu demandes la valeur de nbr à o ou à this.counter_model,
                tu auras toujours exactement la même réponse au même instant.
            Solution :
                arg => Un text précis dans ce contexte.
             */
            if("NBR_A_CHANGE".equals(arg)){
                this.label_text_number.setText( String.valueOf(this.get_counter_model().get_nbr()) );
            }

        }
        // Si o est bien une instance de la classe Modèle dont je veux observer le changement.
        if(o instanceof User){
                // Vérifie ci c'est una variable précise du modèle qui a changée.
                if("NAME_A_CHANGE".equals(arg)) {
                    if( this.get_user().message_error_must_be_display()){
                        // On ajoute le message d'erreur.
                        // Le message d'erreur sera ajouté ET affiché après l'élément qui se trouve à l'index X-1.
                        this.vbox_panel.getChildren().add(1,label_text_error_name_user()); // L'index dans ce cas = numéro de l'étage dans la VBox.
                    }
                }
            }
        }
