...
<?php
echo Navigation::top_bar("item/open_item_view/".$item->get_id(),"Open item","historynav/manage_btn_back/back");
?>
<main class=" p-3">
        ...
        <div class="colonne-droite">   <!--- classe parent qui gére la colonne droite. --->
            ...
            <?php if($item->get_owner_id() == $user->get_id()): ?>
                <div class="manage-your-item card">
                    ...
                    <?php
                    $mon_tableau = ["item/open_item_view/".$item->get_id(), "item/manage_add_edit_item_view/".$item->get_id()];
                    $json = json_encode($mon_tableau);
                    $donnees_codees = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($json));
                    ?>
                    <a href="historynav/manage_btn_back/forward/<?=$donnees_codees?>" class="btn edit-item-details-btn w-100">
                        <i class="bi bi-pen me-2"></i>Edit item Details
                    </a>
                    <?php
                    $mon_tableau = ["item/open_item_view/".$item->get_id(), "item/manage_images_view/".$item->get_id()];
                    $json = json_encode($mon_tableau);
                    $donnees_codees = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($json));
                    ?>
                    <a href="historynav/manage_btn_back/forward/<?=$donnees_codees?>" class="btn manage-images-btn w-100">
                    ...
                    $donnees_codees = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($json));
                    ?>
                    <a href="item/delete/"<?php $item->get_id() ?> class="btn delete-item-btn w-100">
