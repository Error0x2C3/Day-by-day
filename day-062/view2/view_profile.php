...
<div class="page-content py-2">
    ...
    <div class="card bg-secondary">
        ...
        <div class="card-body">
            <?php
            $mon_tableau = ["profile/profile_view", "profile/profil_edit_view/"];
            $json = json_encode($mon_tableau);
            $donnees_codees = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($json));
            ?>

            <a href="historynav/manage_btn_back/forward/<?=$donnees_codees?>" class="btn btn-outline-primary w-100 text-start mb-3">
                <i class="bi bi-person-gear me-2"></i>
                <strong>Edit Profile</strong><br>
                <small class="text-muted">Update your personal information</small>
            </a>
            <?php
            $mon_tableau = ["profile/profile_view", "profile/profile_change_password_view/"];
            $json = json_encode($mon_tableau);
            $donnees_codees = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($json));
            ?>
            <a href="historynav/manage_btn_back/forward/<?=$donnees_codees?>" class="btn btn-outline-primary w-100 text-start mb-3">
            ...
            <?php
            $mon_tableau = ["profile/profile_view", "profile/manage_profile_picture_view/"];
            $json = json_encode($mon_tableau);
            $donnees_codees = str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($json));
            ?>
            <a href="historynav/manage_btn_back/forward/<?=$donnees_codees?>" class="btn btn-outline-primary w-100 text-start mb-3">
