...
class Member extends Model {
    ...
    public function get_visible_messages_as_json(Member $user) : string {
        ...
        foreach ($messages as $message) {
            // le message n'est "supprimable" que si l'utilisateur connecté en
            // est l'auteur ou le destinataire 
            $erasable = $user == $message->author || $user == $message->recipient;
            // le message n'est visible que s'il est public ou, s'il est privé, 
            // si l'utilisateur connecté en est l'auteur ou le destinataire
            if (!$message->private || $erasable){
                $row = [];
                $row["id"] = $message->post_id;
                $row["datetime"] = $message->date_time;
                $row["author"] = $message->author->pseudo;
                $row["body"] = $message->body;
                $row["private"] = $message->private;
                $row["erasable"] = $erasable;
                $table[] = $row;
            }
        }
