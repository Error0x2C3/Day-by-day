...
drop trigger if exists heure_reservation_trigger on reservations;
drop function if exists heure_reservation();
create or replace function heure_reservation() returns trigger as
$$
declare
    _days_of_week int;
    _hour_start time;
...
create trigger heure_reservation_trigger
    before insert or update
    on reservations
    for each row
execute procedure heure_reservation();
