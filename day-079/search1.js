...
$(async function () {
    ...
    if (typeof(word_search) !== 'undefined' && word_search) {
        ...
        await mamange_search(word_search, list_balises);

    }
    // Si je tape quelque chose dans la barre de recherche.
    $('#fulltext-search').on('input', async function () {
        await mamange_search($(this).val().toLowerCase(), list_balises); // On récupére la version minuscule du mot.
    });
});

// Gestion du processus de recherche du mot.
async function mamange_search(search_content,list_balises){
