...
class ControllerHistoryNav extends Controller {
    ...
    public static function get_tab_links():array{
    ...
    public function manage_btn_back(): void{
        ...
        if($option == "back"){
            ...
            $link_prev = $this->back();
            if($link_prev != null){
                $link = explode('/', $link_prev);
                /*
                Cas basique :
                    Sortie de $link-prev pour un back simple :
                    Array (
                        [0] => "item"
                        [1] => "browse_items_view"
                    )
                 */
                /*
                Cas avec un arguement :
                    Ex :
                        Cas ou il était dans la page manage image et retourne dans open item,
                        open item a besoin d'avoir en url l'id de l'item.
                 */
                if(isset($_GET["param2"])){
                    $this->redirect($link[0],$link[1],$_GET["param2"]);
                }
    ...
    public function forward($link_prev, $link_next): void{
        ...
        */
        ...
        if($link[2] != null){
            /*
            Je passe un argument dont la méthode de la page suivante à besoin.
            Ex :
                ["item/browse_items_view", "item/open_item_view/".$item->get_id()],
                Sortie de $link_next :
                "item/open_item_view"
                Array (
                    [0] => "item"
            ...
            */
            $this->redirect($link[0],$link[1],$link[2]);
    ...
    public function back():string|null{
        ...
        */
        ...
        // array_pop fonctionne sur la session.
    ...
    */
    ...
    B)
     --- Explication ---
        Bien que la syntaxe PHP self::$tab_links[] = $link_prev;
        soit correcte pour ajouter un élément à un tableau statique,
        il y a un problème fondamental lié au fonctionnement du
        protocole HTTP et du cycle de vie de PHP.

        Problème :
             La persistance (Stateless).
             PHP est "stateless" (sans état). Voici ce qui se passe réellement :
                  1 ) L'utilisateur clique sur un lien.
                  2)  Le serveur lance le script PHP, crée la classe,
                      exécute forward(), remplit le tableau statique $tab_links,
                      puis effectue une redirection ($this->redirect(...)).
                  3)  Le script s'arrête immédiatement.
                      Toute la mémoire (y compris le tablea statique $tab_links) est effacée.
                  4)  Le navigateur demande la nouvelle page.
                      Un nouveau processus PHP démarre avec un tableau self::$tab_links totalement vide.
                      L'historique sera donc systématiquement perdu entre deux pages.
        Solution :
             Pour que votre historique survive entre
             les changements de pages, vous devez stocker le tableau
             dans la superglobale $_SESSION.
             Pour ça il faut que le constructeur ou qu'à chaque début de mes méthodes,
             je m'assure que la session contient un tableau pour l'historique :
