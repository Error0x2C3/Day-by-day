...
class Items extends Model {
    ...
    */
    ...
    public function should_close(?float $highest_bid): bool {
        ...
        // 1) Sinon clôture à la fin de période si c’est une enchère OK.
    ...
    /*
    La première enchère qui atteint ou dépasse le prix d'achat immédiat remporte l'annonce et clôture les enchères.
     */
    public static  function firstBidsReachingBuyNow(): bool{
        $picture_path ="";
        $iban ="";
        $role ="user";
        // Sans try/catch, une requête SQL peut échouer et le code continuer comme si de rien n’était.
        try {
            self::execute("Select * from items where owner = :owner",
                array(
                    "full_name"=> $full_name,
                    "email"=>$email,
                    "pseudo"=>$pseudo,
