...
class Items extends Model {
    ...
    */
    ...
    // Permet de savoir s'il y a un gagné durant les enchères, si oui qui.
    public function has_a_winner():bool|int{
        ...
        if($this->has_auction() && $this->has_buy_now()){
            if($this->getFirstWinningBuyNowBid($this->get_id()) instanceof Bids && $this->has_buy_now()){
                // On renvoie l'Id du propriètaire de l'enchère.
                return $this->getFirstWinningBuyNowBid($this->get_id())->owner_id;
            }
            return false;
        }
        // 2) Si à la fin de la période d'enchères aucune
        //    enchère n'a atteint le prix d'achat immédiat,
        //    l'annonce est remportée par l'enchère la plus élevée.
        if($this->has_auction() && $this->time_has_passed() && !$this->getFirstWinningBuyNowBid($this->get_id())){
    ...
    public function should_close(): bool {
        ...
        if ($this->has_auction() && $this->time_has_passed()) {
            return true;
        }
        ...
        if($this->has_auction() && $this->has_buy_now()){
             ...
             */
            if($this->getFirstWinningBuyNowBid($this->get_id()) instanceof Bids){
                return true;
            }

        }
        //TODO
        // 3) L'annonce peut éventuellement contenir un prix d'achat immédiat,
        //    auquel cas un utilisateur peut décider d'acheter immédiatement
        //    l'annonce au prix indiqué. Un achat immédiat se concrétise
        //    par la création d’une offre d'achat (enchère) dont le montant est supérieur ou égal à buy_now_price
