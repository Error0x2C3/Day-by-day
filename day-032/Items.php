...
//    }
   ...
   ========================================================= */
    ...
    public function max_bid():bool | int {
        try {
            $pdo = self::execute("SELECT MAX(b.amount) FROM bids b
                                      JOIN items i ON b.item = i.id
                                      WHERE b.item = :itemId
                                ",
                array(
    ...
    public function is_auction():bool{
        if($this->has_auction()){return true;}
        return false;
    }

    // Un booléen indiquant si l'annonce a reçu des offres d'achat.
    public function has_bids():bool{
    ...
    public function buy_now_reached():bool | int{
    ...
    public function not_purchased_direct_sale():bool {
        if($this->first_bid_for_buy_price_now_only() instanceof Bids){
            return false;
        }
        return true;
