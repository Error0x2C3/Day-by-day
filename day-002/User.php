...
class User extends Model {
    ...
    public function set_iban(string $iban): void{
            ...
            if($file['error']== 0){
                if($this->check_format_picture($file)){
                    if($file['size'] <=Configuration::get("picture_size_max")){
                        ...
                        $error['image_size'] = "La taille de l'image n'est pas bonne.";
                    }
                }else{
                    $error['format'] = "Format non autorisé.";
                }
            } else{
                $error['upload'] = "Problème lors du changement de l'image.";
            }
        }else{
            $error["fichier_transmis"] = "Pas de fichier transmis.";
        }
        if(empty($error)){
            return true;
        }
