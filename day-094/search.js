...
/*
Le bug des doublons vient d'un conflit de vitesse dans dans js/search.js.
Voir doc_debug_search_js
 */
let currentSearchId = 0; // variagle globale.
...
async function mamange_search(search_content,list_balises, search_bar_html){
    ...
    // Les deux balises de Browse item pour le cas Iitem no found.
    let bool_participating_browse_item_no_found =false;
    let bool_avaible_items_browse_item_no_found =false;
    // Les trois balises de My item pour le cas No item found.
    let bool_active_items_item_no_found = false;
    let bool_closed_unsold_items_item_no_found = false;
    let bool_sold_items_item_no_found = false;
    ...
    for(let i=0 ; i <= list_balises.length-1 ; i++){
        ...
        if(i===0){ // 0 correspond à Items I'm Participating In de Browse Item.
            ...
            }else{ bool_participating_browse_item_no_found= true;}
            ...
            }else{bool_avaible_items_browse_item_no_found =true;}
            ...
            }else{ bool_active_items_item_no_found=true;}
            ...
            }else{bool_closed_unsold_items_item_no_found=true;}

        }else if(i ===4){ // 4 correspond à Closed Sold items de My items.
            ...
            }else{ bool_sold_items_item_no_found=true;}
        }
    }
    // S'il on a plus aucun item correspondant à notre recherche pour la page Browse-item.
    if(bool_participating_browse_item_no_found && bool_avaible_items_browse_item_no_found){ // Pour Browse_item.
        console.log("test 1");
        $("#bar_search").children().slice(1).hide(); // On cache tous les enfants sauf le premier qui est la barre de recherche.
        if($("#bar_search").find("#no_item_found").length ===0) {
            $("#bar_search").append(create_page_no_found()); // on ajoute le Item no Found.
        }
        // S'il on a plus aucun item correspondant à notre recherche pour la page My items.
    }else if(bool_active_items_item_no_found && bool_closed_unsold_items_item_no_found && bool_sold_items_item_no_found){
        $("#bar_search").children().slice(1).hide(); // On cache tous les enfants sauf le premier qui est la barre de recherche.
        if($("#bar_search").find("#no_item_found").length ===0) {
            $("#bar_search").append(create_page_no_found()); // on ajoute le Item no Found.
        }
    }else {
        $("#no_item_found").remove(); // Supprime No item found de la page.
        $("#bar_search").children().slice(1).show(); // Affiche tous les enfants (sauf le premier) qui étaient cachés.
    }
