...
//                "path_photo"=>$user->get_picture_path()
    ...
    public function sales_view():void{
        ...
        $word_sale = count($list_items)> 1 ? count($list_items)." sales" : "1 sales" ;
        $sum_revenue = 0;
        foreach($list_items as $item){
            $sum_revenue += $item->get_max_bid();
        }
        $average_ticket = $sum_revenue/count($list_items);
        $recurring_winning = User::get_user_by_id($list_items[0]->get_owner_id())->get_pseudo();
            (new View("sales"))->show(
            [
                "user" => $user,
