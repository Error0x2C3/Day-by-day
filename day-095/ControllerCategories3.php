...
class ControllerCategories extends Controller {
    ...
    public function save(categories $category, String $category_name){
        ...
        // Cas ou modification du nom de la catégorie.
    ...
    public function update_category_service() {
        ...
        $category = categories::get_categorie_by_id($_POST['id_category']);
        $new_name_category = isset($_POST['new_name_category']) ? $_POST['new_name_category'] : "";
        if($user->get_role() === Role::Admin && $category instanceof categories){
            $category_name = $category->get_name();
            if(categories::is_not_already_in_use($new_name_category) && $new_name_category !== $category->get_name() && Configuration::get("name_categorie_min")<= strlen($new_name_category) && Configuration::get("name_categorie_max") >= strlen($new_name_category)){
                $category->set_name($new_name_category);
                if($category->save()){
                    echo json_encode(["success" => true]);
                    exit;
                }
            }else{
                echo json_encode(["success" => false, "message" => "Le nom est déjà pris !"]);
                exit;
            }
        }
    }
    ...
    public function delete_category_confirm_view(){
        ...
        if($category instanceof categories){
            if(count(item_categories::get_items_category_by_category_id($category->get_id())) == 0 ){
    ...
    public function add_service(){
        ...
        $user = $_SESSION["user"];
        if($user->get_role() === Role::Admin){
            $new_name_categorie = isset($_POST['add_new_category_input_name']) ? $_POST['add_new_category_input_name'] : "";
            $error_array_new_category["error"] = "This name is already considered a category ! Or \n The category must contain between 3 and 25 characters";
            if(categories::is_not_already_in_use($new_name_categorie) && Configuration::get("name_categorie_min")<= strlen($new_name_categorie) && Configuration::get("name_categorie_max") >= strlen($new_name_categorie)){
                if(categories::add($new_name_categorie)){
                    echo json_encode(["success" => true]);
                ...
                echo json_encode(["success" => false, "message" => "This name is already considered a category ! Or \n The category must contain between 3 and 25 characters"]);
                exit;
            }
        }
    }
    public function get_all_categories_and_co(){
        $this->check_user_session();
        $user = $_SESSION["user"];
        $list_categories = categories::get_all_service();
        // Ajoute des informations supplémentaires dans la liste.
        foreach ($list_categories as &$category) {
            $category["nbr_items_for_category"] = count(item_categories::get_items_category_by_category_id((int)$category["id"]));
        }
