   ...
   ========================================================= */
    ...
    public static function other_available_items($user_id){
        try {
            $pdo = self::execute("
               SELECT DISTINCT s1.*
                    FROM v_items_status s1
                    WHERE s1.owner != :user_id
                    AND s1.id NOT IN (
                        SELECT b2.item FROM bids b2
                        WHERE b2.owner = :user_id2
                    )
                    AND s1.end_at > :now
                    AND s1.buy_now_reached = 0
                    AND NOT (s1.is_direct_sale = 1 AND s1.has_bids = 1)
