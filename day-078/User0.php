...
class User extends Model {
    ...
    public function get_role_to_strng(): string {
    ...
    public function get_picture_path(): ?string {
        ...
        // Ex : /prwb_2526_f09/uploads/users/4/profile.jpg;
        return  Configuration::get("web_root") .$this->picture_path;
    }
    ...
    public function set_role_without_validate(Role $role){
         $this->role = $role;
    }
    ...
    public static function get_user_by_id(int $id): User|bool {
        ...
        if ($user !== false) {
            return new User(
                ...
                role:       $user["role"] === "admin" ? Role::Admin : Role::Member,
    ...
    public static function get_instance_user(string $email, string $password): User|bool {
        ...
        if($user !== false &&  password_verify($password, $user["password"]) == true){
            return new User(
                ...
                pseudo: $user["pseudo"] ?? "",
                password: $user["password"],
                role: $user["role"] ==  Role::from("Member") ? Role::Member : Role::Admin,
                picture_path: $user["picture_path"] ?? "",
                iban: $user["iban"] ?? "",
                id : $user["id"],
            );
   ...
   ========================================================= */
    ...
    public static function validate_role($role): bool {
         ...
         */
         if(Role::from($role) == Role::Admin || Role::from($role) == Role::Member){
            return true;
         }
        return false;
    }
    ...
    */
    private function create_picture(Array $file): bool {
        ...
        $upload_dir = "uploads/users/".$this->get_id()."/";
  ...
  ========================================================= */
    ...
    */
    public function get_user_items_participing():bool|array{
        try {
            $pdo = self::execute("SELECT DISTINCT s.*
                FROM v_items_status s
                JOIN bids b ON b.item = s.id
    ...
    // Donne tous les items de la BDD auxquels l'utilisateur courrant participent au format json.
    public function get_user_items_participing_search_service( String $word):bool|array{
        try {
            $pdo = self::execute("SELECT DISTINCT s.*
    FROM v_items_status s
    JOIN bids b ON b.item = s.id
