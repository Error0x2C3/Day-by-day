...
class Items extends Model {
    ...
    public function validate_created_at(DateTime $dateTime):bool{
        $now = new DateTime();
        if($dateTime >= $now){
            return true;
        }
    ...
    */
    ...
    // Permet de savoir si l'annonce a commencé.
    public function has_started():bool {
        $now = new DateTime();
        if( $this->get_created_at() >= $now){
            return true;
        }
        return false;
    }
    // Permet de savoir si l'annonce doit être cloturée ou pas.
    ...
    // Donne la date de fin de la mise enchère.
