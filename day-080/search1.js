...
$(async function () {
    ...
    // Si on est dans le cas ou on revient d'une recherche.
...
async function mamange_search(search_content,list_balises, search_bar_html){
    ...
    for(let i=0 ; i <= list_balises.length-1 ; i++){
        ...
        if(i===0){ // 0 correspond à Items I'm Participating In de Browse Item.
            ...
            if( resultat_search_word && resultat_search_word.length > 0){
...
async function search_filter_list_items_user(word, mode) {
    try {
        ...
        let response = await $.ajax({
            ...
            type: 'POST',
            data: { word_search: word,mode:mode},
            dataType: 'json'
        });
...
function create_page_no_found(){
    return `
        <div class="empty-state-container d-flex justify-content-center align-items-center">
...
const monTableau = JSON.parse(texteJson);
 */