 ...
 // Les annonces.
   ...
   ========================================================= */
    ...
    public function validate_title(
        ...
        // 1️⃣ Vérifie longueur minimale
        if (strlen(trim($title)) < (int)Configuration::get("title_item_min")) {
            return false;
        }
        echo $check_unique;
        // 2️⃣ Si on ne doit PAS vérifier unicité (edit)
        if (!$check_unique) {
            return true;
        }
        ...
        // 3️⃣ Vérifie unicité
        $pdo = self::execute(
            "SELECT COUNT(*) FROM items WHERE owner = :owner AND title = :title",
            [
                "owner" => $owner_id,
                "title" => $title
            ]
    ...
    public static function get_user_item_picture_path_and_priority(int $item_id): array
{
   ...
   ========================================================= */
    ...
    // Obtenir les items disponiblesauquelles l'utilisateur ne participent pas.
