...
class ControllerCategories extends Controller {
    ...
    public function categories_view(){
        ...
        $error_array_save_category = isset($_GET['param1']) ? Navigation::decode($_GET['param1']) : [] ;
    ...
    public function save(categories $category, String $category_name){
        ...
        if(categories::is_not_already_in_use($category_name) && $category_name !== $category->get_name()){
            $category->set_name($category_name);
            $category->save();
            $this->redirect("categories","categories_view");
        // Si rien n'est modifier.
        }else if($category->get_name() === $category_name){
            $this->redirect("categories","categories_view",Navigation::encode($error_array_save_category));
        }else{
            $error_array_save_category["error"] = "This name is already considered a category !";
            $this->redirect("categories","categories_view",Navigation::encode($error_array_save_category));
        }
    }
