...
<!-- Bootstrap JS (inclut Popper) — requis pour bootstrap.Carousel -->
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
<!-- jQuery — requis pour $(), $.getJSON(), $.each() -->
<script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>
<script>
$(function () {
    var $zone = $('#js-image-zone');
    var itemId = $zone.data('item-id');
    $.getJSON('Item/get_images_service/' + itemId, function (data) {
        /*if (!data.images || data.images.length === 0) {
            // Pas d'image : afficher le placeholder
            $('#product-images-card').show().find('.carousel').replaceWith(
        ...
        $.each(images, function (i, src) {
            ...
            $inner.append(
                '<div class="carousel-item' + activeClass + '">' +
                    '<img class="main-img d-block w-100" src="' + src + '" alt="">' +
                '</div>'
            );
            console.log("ajouter")
        });

        $('#product-images-card').show();

        // -- Construire les vignettes --
        if (images.length > 1) {
            var $grid = $('#galerie-grid');
            console.log('carousel-inner trouvé :', $grid.length); // doit afficher 1
console.log('contenu avant :', $grid.html());
            $grid.empty();

            $.each(images, function (i, src) {
                var $thumb = $('<img class="thumb-img" src="' + src + '" alt="" style="cursor:pointer;">');

                // Clic vignette → slide carousel sur l'image correspondante
                $thumb.on('click', (function (index) {
                    return function () {
                        var carousel = bootstrap.Carousel.getOrCreateInstance(
                            document.getElementById('carouselItemImages')
                        );
                        carousel.to(index);
                    };
                })(i));
...
if(isset($item) && isset($btn_back) && count($btn_back) >0){
    echo Navigation::top_bar("item/open_item_view/".$item->get_id(),"Open item",$btn_back[0]);
}
?>
<?php
?>
<main class=" p-3">
    <div class="page-content py-2"> <!---classe parent qui gére la grille, le container. --->
        <div class="colonne-gauche">  <!--- classe parent qui gére la colonne gauche. --->
            ...
            <div id="js-image-zone" data-item-id="<?= $item->get_id() ?>"></div>
                <?php if (!Configuration::get("disable_js")):?>
                    <div class="product-images card" style="padding: 0;">
                    <div id="carouselExample" class="carousel slide product-images" style="padding: 0;">
                ...
                <div class="product-content p-4">
                    <div class="description-1">
                        ...
                        <?php if($item->has_buy_now() && !$item->has_auction()):?>
                            <span class="badge bg-secondary">Direct sale</span>
                        <?php else : ?>
                    ...
                    <div class="categorie">
                        ...
                        <ul>
                        <?php if (isset($item_categorie) && !empty($item_categorie) ) :?>
                            
                            <?php foreach($item_categorie as $categorie): ?>
                                    <li><?php echo $categorie->get_name() ?></li>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </ul>
                    ...
                    <div class="product-dates">
                        <?php if($item->has_auction() && $item->has_buy_now() || $item->should_close() || $item->get_owner_id() == $user->get_id()):?>
            ...
            <?php if(!empty($list_pictures_item) && count($list_pictures_item) > 1): ?>
                ...
                <?php else : ?>
                    <!-- Galerie de vignettes JS — cachée, JS la remplit et l'affiche -->
                    <div id="galerie-js" class="galerie-picture card" style="display: none;">
                        <h3>Additional Images</h3>
                        <div class="galerie-picture-grid" id="galerie-grid">
                            <!-- Les vignettes seront injectées ici par le JavaScript -->
                        </div>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
            ...
            <!--Bid History-->
            <?php if($item->get_bid_count() > 0) :?>
                <div class="history-bids card"> <!--- Classe card => style css | class history-bids => identité --->
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h3>Bid History</h3>
                        <?php
                            $word = $item->get_bid_count() > 1 ?  $item->get_bid_count()." "."entries" :  $item->get_bid_count()." "."entry";
                        ?>
                    ...
                    <div class="history-list">
                        <?php foreach ($item->get_bid_history() as $bid_item) { ?>
                            <div class="history-row">
                                ...
                                <div class="bid-price"><span>€ </span><?php echo  number_format($bid_item->get_amount(), 2, ',', '.');?></div>
                            </div>
                        <?php }?>
                    </div>
                </div>
            ...
            <div class="pricing card ">
                ...
                <?php if(!$item->should_close()):?>
                    ...
                    <?php if($item->get_owner_id() != $user->get_id()): ?>
                        ...
                        <?php elseif($item->has_buy_now() && !$item->has_auction() && !$item->is_has_bids()):?>
                            <div class="bid-row">
                                <p>Price</p>
                                <p class="buy-row-price-only"> <span>€</span><?php echo  number_format($item->get_buy_now_price(), 2, ',', '.');?></p>
                            </div>
                        <?php endif; ?>
                        <!--- AFFICHAGE DES PRIX -->
                        <!--- AFFICHAGE DES PRIX -->
                ...
                <form action="item/buy_bid/<?= $item->get_id() ?>" method="POST">
                    ...
                    <?php if(!$item->should_close()):?>
                        ...
                        <?php if($item->get_owner_id() != $user->get_id()): ?>
                            ...
                            <?php if(!$item->is_has_bids() && !$item->has_buy_now() && $item->has_auction()):?>
                                <div class="custom-input-group">
                                    ...
                                    <input name="bid_amount" type="number" value="<?= number_format($item->get_starting_bid()+1, 2, ',', '.');?>"step="any">
                                </div>
                            <!-- 1) BIS A déjà des enchères et pas d'achat immédiat.-->
                            <?php elseif($item->has_auction() && $item->is_has_bids() && !$item->has_buy_now()):?>
                                <?php $resultat = $item->get_max_bid() !== null ? $item->get_max_bid() : $item->get_starting_bid() ; ?>
                                <div class="custom-input-group">
                                    <span class="input-prefix">€</span> <!-- Input Group/(groupe de saisie) -->
                                    <input name="bid_amount" type="number" value="<?=number_format($resultat+1, 2, ',', '.'); ?>"step="any">
                                </div>
                            <!-- 2) Enchères existantes, prix d'achat immédiat non atteint. -->
                            ...
                            <!-- 3) Vente directe uniquement et n'a pas déjà reçu une enchère. -->
                            <?php elseif($item->has_buy_now() && !$item->has_auction() && !$item->is_has_bids()):?>
                                <!-- Pas besoin de choisir un moment dans un achat immédiat.-->
                            <?php else : ?>
                                <div class="custom-input-group">
                                    <span class="input-prefix">€</span> <!-- Input Group/(groupe de saisie) -->
                                    <input name="bid_amount" type="number" value="<?= number_format($item->get_starting_bid(), 2, ',', '.');?>"step="any">
                                </div>
                            <?php endif; ?>
                            <!-- BARRE POUR TAPER LE PRIX. -->
                            <!-- BARRE POUR TAPER LE PRIX. -->
                        ...
                        <?php elseif($item->get_owner_id() == $user->get_id()) : ?>
                            <!-- Vente directe uniquement et n'a pas déjà reçu une enchère. -->
                            <?php if($item->has_buy_now() && !$item->has_auction()):?>
                                <button type="submit" name="action" value="buy_now" class="btn buy-now-direct-only w-100" disabled>
                            ...
                            <?php elseif($item->has_auction() && !$item->has_buy_now()):?>
                                <div class="custom-input-group">
                                    <span class="input-prefix">€</span> <!-- Input Group/(groupe de saisie) -->
                                    <input name="bid_amount" type="number" value="<?= number_format($item->get_starting_bid(), 2, ',', '.');?>"step="any"  disabled>
                                </div>
                                <button type="submit" name="action" value="bid" class="btn bid-direct-only-owner w-100" disabled>Place Bid</button>
                            ...
                            <?php elseif($item->has_auction() && $item->has_buy_now()):?>
                                ...
                                <button type="submit" name="action" value="bid" class="btn bid-direct-only-owner w-100" disabled>Place Bid</button>
                                <button type="submit" name="action" value="buy_now" class="btn buy-now-direct-only-owner w-100" disabled>Buy Now at<span>€</span><?php echo number_format($item->get_buy_now_price(), 2, ',', '.');?></button>
                ...
                <?php if(!$item->should_close()):?>
                    ...
                    <?php if($item->get_owner_id() != $user->get_id()): ?>
                        ...
                        <?php elseif($item->is_has_bids() && $item->has_buy_now() && !$item->get_first_winning_buy_now_bid()): ?>
                            <button type="submit" name="action" value="bid" class="btn bid-row-btn w-100">Place Bid</button>
                            <button type="submit" name="action" value="buy_now" class="btn buy-row-btn w-100">Buy Now at <span>€</span><?php echo  number_format($item->get_buy_now_price(), 2, ',', '.');?></button>
                        <!-- 2) BIS Enchères existantes, prix d'achat immédiat non atteint. -->
                        <?php elseif($item->has_auction() && !$item->is_has_bids() && $item->has_buy_now() && !$item->get_first_winning_buy_now_bid()): ?>
                            <button type="submit" name="action" value="bid" class="btn bid-row-btn w-100">Place Bid</button>
                            <button type="submit" name="action" value="buy_now" class="btn buy-row-btn w-100">Buy Now at <span>€</span><?php echo number_format($item->get_buy_now_price(), 2, ',', '.');?></button>
                    ...
                    <?php endif; ?>
                    <!-- Pour le bouton back-->
                    <input type="hidden" name="btn_back" value="<?=Navigation::encode($btn_back)?>">
                </form>
                <!--Cas ou l'offre est inférieur à l'enchère en cours et ou à l'achât immédiat.-->
                <?php if (isset($error)) :?>
            ...
            <div class="seller-information card ">
                ...
                <div class="seller-details">
                    ...
                    <div class="info-profile">
                        ...
                        <p><?php echo  $owner_user->get_role_to_string(); ?></p>
                    </div>
                </div>
            </div>
            <?php if($item->get_owner_id() == $user->get_id() && isset($item)): ?>
                <div class="manage-your-item card">
                    <h3>Manage Your Item</h3>
                    <?php
                    $btn_back[]="item/open_item_view/".$item->get_id()."/".Navigation::encode($btn_back);
                    ?>
                    <a href="item/edit_item_view/<?=$item->get_id()?>/<?=Navigation::encode($btn_back)?>" class="btn edit-item-details-btn w-100">
                        <i class="bi bi-pen me-2"></i>Edit item Details
                    </a>
                    <a href="item/manage_images_view/<?=$item->get_id()?>/<?=Navigation::encode($btn_back)?>" class="btn manage-images-btn w-100">
                        <i class="bi bi-image me-2"></i>Manage Images
                    </a>
                    <a href="item/delete_confirm_item_view/<?=$item->get_id()?>/<?=Navigation::encode($btn_back)?>" class="btn delete-item-btn w-100">
