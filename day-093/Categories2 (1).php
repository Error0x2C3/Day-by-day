...
Class categories extends Model{
    ...
    public function set_name(String $category_name){
        if(categories::is_not_already_in_use($category_name) && Configuration::get("name_categorie_min")<= strlen($category_name) && Configuration::get("name_categorie_max")>= strlen($category_name) ){
    ...
    public static function add($name): bool {
        try {
            $pdo = self::execute("
                ...
                array(
                    "name" => $name,
                    "priority" => self::get_priority_max()+1
                )
            );
            if (!$pdo) {
                return false;
