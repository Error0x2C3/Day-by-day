...
class ControllerItem extends Controller {
    ...
    */
    ...
    public function edit_item_view(): void{
        ...
        if(isset($_GET['param1'])){
            ...
            $item_categories  = Items::get_item_categori($item->get_id()) ?: [];
            $selected_cat_ids = array_map(fn($c) => $c->get_id(), $item_categories);
    ...
    public function save_item_from_form(){
        ...
        $selected_cat_ids = isset($_POST['categories']) ? array_map('intval', (array)$_POST['categories']) : [];
        $all_categories   = categories::get_all() ?: [];
        $errors = [];
        if (count($selected_cat_ids) > 3) {
            $errors["categories"] = "Vous ne pouvez pas sélectionner plus de 3 catégories.";
        }
        if (!empty($errors)) {
        ...
        try {
            ...
            $item->save();
            Items::set_item_categories((int)$item_id, $selected_cat_ids);
    ...
    public function add_item_from_form(): void {
        ...
        if (is_int( $new_item_id)) {
            Items::set_item_categories($new_item_id, $selected_cat_ids);
    ...
    public function update_order_service() {
    ...
    public function browse_items_view(): void{
        ...
        $user ="";
        if( $this->get_user_or_false() && ( $user = User::get_user_by_id($_SESSION["user"]->get_id()) instanceof User) ){
            // User::get_user_by_id($_SESSION["user"]->get_id()) => Pour forcer la recharge des données dans la BDD.
            $user = User::get_user_by_id($_SESSION["user"]->get_id());
   ...
   ========================================================= */
    ...
    public function open_item_view():void {
        ...
        $btn_back= isset($_GET['param2']) ? Navigation::decode($_GET['param2']) : "";
        $categorie = Items::get_item_categori($item->get_id());
        $success = isset($_GET['param3']) ? Navigation::decode($_GET['param3']) : null;
    ...
    public function buy_bid():void{
        ...
        if(isset($_GET['param1'])){
            ...
            if($item->get_owner_id() != $user->get_id()){
                try {
                    // Est Capitale pour appeler la méthode  validate_construct() qui rélève les erreurs.
                    ...
                    $this->redirect("item","open_item_view",$item->get_id(),Navigation::encode($btn_back),Navigation::encode($success));
                }catch(InvalidArgumentException $e) {
                    // On attrape le message d'erreur définit dans la méthode validate_construct() du modèle Bids.
                    $error = $e->getMessage();
                    (new view("open_item"))->show([
                        "user" => $user,
                        "owner_user" => $owner_user,
                ...
                $this->redirect("item","open_item_view",$item->get_id(),Navigation::encode($btn_back));
            }
        }
    }
    /* =========================================================
...
//    }
  ...
  ========================================================= */
    ...
    /* =========================================================
      Delete confirmation
   ========================================================= */
    public function delete_confirm_item_view(): void{
        $this->check_user_session();
        $user = $_SESSION["user"];
        // $item->delete();
        ...
        (new View("Delete_item_confirm"))->show(
            [
                "item" => $item,
                "owner" => $owner,
                "user" => $user,
    ...
    public function delete_item():void{
        ...
        $user = $_SESSION["user"];
        $item = isset($_GET['param1']) ? Items::get_Item_instance_by_id(intval($_GET['param1'])): null;
        if($user->get_id() === $item->get_owner_id()){
            $item->delete();
   ...
   ========================================================= */
    ...
    public function get_card_search_service(): void{
        ...
        if(isset($word)){
            ...
            // Filtrage par catégorie (post-traitement)
            // Lorsqu'on revient d'un page :
            if($category_id !== null){
                // array_filter, pour chaque élément de la list elle exécute la fonction anonyme.
                $list = array_filter($list, function($item) use ($category_id){
                    // Pour avoir l'id de la catgérie auquelle appartient l'item.
                    $cat_ids = Items::get_category_ids_for_item($item["id"]);
                    // Vérigie si l'id de chaque item correspond avec celuit de $categorie_id.
                    return in_array($category_id, $cat_ids);
                });
                $list = array_values($list);
            }

            // Propagation : on encode [word, category_id] pour le btn_back.
            $propagation = [$word, $category_id];

            // Ajoute des informations supplémentaires dans la liste.
            foreach ($list as &$item) {
                ...
                if($mode === "participating" || $mode === "available-items"){
                    $btn_back[] = "item/browse_items_view/".Navigation::encode($propagation);
                    ...
                    $btn_back[] = "item/my_items_view/".Navigation::encode($propagation);;
                $item["btn_back"] = Navigation::encode($btn_back);
                }
            }
