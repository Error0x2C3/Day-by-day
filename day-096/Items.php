 ...
 // Les annonces.
   ...
   ========================================================= */
    ...
    public function set_title($title): bool{
        if($this->validate_title($title, $this->owner_id)){
            $this->title = $title;
            return true;
        }
        return false;
    ...
    public  function validate_contruct(string $title,?String $description,DateTime $created_at,int $duration_days,int $owner_id,?float $buy_now_price,?float $starting_bid,?bool $check_unique=true):void{
        if (!$this->validate_title($title, $owner_id, $check_unique)) {
            throw new InvalidArgumentException("Le titre n'est pas bon.");
        }
    ...
    public function validate_title(
        ...
        // 1 Vérifie longueur minimum.
        if (strlen(trim($title)) < (int)Configuration::get("title_item_min")) {
            return false;
        }
        // 2 Si on ne doit PAS vérifier unicité (edit).
        if (!$check_unique) {
            return true;
        }

        // 3 Vérifie unicité.
        $sql = "SELECT COUNT(*) FROM items WHERE owner = :owner AND title = :title";
        $params = [
            "owner" => $owner_id,
            "title" => $title
        ];

        // Exclure l'item actuel s'il existe (pour l'édition).
        if (isset($this->id) && $this->id > 0) {
            $sql .= " AND id <> :id";
            $params["id"] = $this->id;
        }

        $pdo = self::execute($sql, $params);
        return $pdo->fetchColumn() == 0;
    }
    public function verify_title_longeur(string $title): ?string {
    ...
    */
    ...
    // Permet de savoir s'il y a un gagnant durant les enchères, si oui qui.
    ...
    public function first_bid_for_starting_bid_only():bool|Bids
    {
        try {
            $pdo = self::execute("SELECT b.* FROM bids b
                                ...
                                /* trie par montan le plus élevé + si éaglité prend le plus récent.*/
                                ORDER BY b.amount DESC, b.created_at ASC
                                LIMIT 1",
                array(
                    "itemId" => $this->get_id(),
    ...
    */
    ...
    private function process_and_save_image($tmp_name, $dest_path, $max_size, $quality = 80):bool {
        ...
        if ($is_thumbnail) {
            ...
            $new_width = $max_size;
            $new_height = $max_size;
            $src_size = (int)min($width, $height);
            $src_x = (int)($width - $src_size) / 2;
            $src_y = (int)($height - $src_size) / 2;
            $src_w = (int)$src_size;
            $src_h = (int)$src_size;
        }else{
            if ($width > $height) {
        ...
        imagecopyresampled($destination, $source, 0, 0, (int)$src_x, (int)$src_y, (int)$new_width, (int)$new_height, (int)$src_w, (int)$src_h);
...
//    }
    ...
    public static function get_item_categori($id){

        try {
            $pdo = self::execute("Select * from item_categories,categories where item_categories.category = categories.id and item = :id  ORDER BY priority ASC;
            ",
                array(
                    "id" => $id
                ));
    ...
    public function delete():bool{
        try {
            // 1. Suppression des catégories liées à l'item.
            self::execute("DELETE FROM item_categories WHERE item = :id", [
                "id" => $this->get_id()
            ]);

            // 2. Suppression des photos liées à l'item dans la BDD.
            self::execute("DELETE FROM item_pictures WHERE item = :id", [
                "id" => $this->get_id()
            ]);
            // 3. Suppression des enchères liées à l'item.
           self::execute("DELETE FROM bids WHERE item = :id", [
               "id" => $this->get_id()
           ]);
            // 4. Suppression de l'item elle-même.
            self::execute("DELETE FROM items WHERE id = :id", [
                "id" => $this->get_id()
            ]);

            // 5. Suppression physique du dossier d'images sur le disque.
            $upload_dir = "uploads/items/" . $this->get_id() . "/";
    ...
    // Fonction utilitaire pour supprimer un dossier et tout ses images.
    private function delete_directory(string $dir): void {
        // Si le chemin du fichier n'existe pas, on sort.
        if (!is_dir($dir)) return;
        /*
        scandir($dir) :
            Cette fonction liste tout ce qui se trouve dans le dossier.
            Le problème, c'est qu'elle inclut toujours deux dossiers cachés du système :
                . (le dossier actuel) et .. (le dossier parent).
        array_diff(...) :
            Elle exclut . et .. de la liste. Sans cela,
            la fonction tournerait en boucle infinie ou tenterait de supprimer le dossier parent !
            Il ne reste donc dans $files que les vrais fichiers et sous-dossiers.
         */
        $files = array_diff(scandir($dir), array('.', '..'));
        foreach ($files as $file) {
            /*
            si c'est un dossier => appelle récursif dessus pour supprimer les fichiers à l'intérieur,
            si c'est un fichier il est supprimer.
            */
            (is_dir("$dir/$file")) ? $this->delete_directory("$dir/$file") : unlink("$dir/$file");
        }
        // rmdir() ne peut supprimer qu'un dossier vide.
        rmdir($dir);
    }
    ...
    // Ajoute une nouvelle item dans la BDD.
    ...
    // Mets à jour les paramètres de l'item
    ...
    // Créer un nouvel item ex dans add_item et l'insérer dans la BDD et renvoyer son id.
    public static function create_new_item(
        ...
        try {
            ...
            return self::lastInsertId(); // Retourne l'ID qui vient d'être créé.
        }catch (PDOException $e) {
            return false;
        }
    ...
    public static function get_user_item_picture_path_and_priority(int $item_id): array
{
    ...
    */
    ...
    // Récupére les annonces actifs qui sont détenue par l'utilisateur courant.
    public static function get_active_items_by_owner(int $owner):bool|array{
        try {
            $pdo = self::execute(
                "SELECT DISTINCT v.*, ip.picture_path
    ...
    // Récupére les annonces actifs qui sont détenue par l'utilisateur courant pour le service JS.
    public static function get_active_items_by_owner_service(int $owner,string $word):bool|array{
        try {
            $pdo = self::execute(
                "SELECT DISTINCT v.*, ip.picture_path
             FROM v_items_status v
            ...
            AND (
                ...
                OR LOWER(v.description) LIKE LOWER(:word)
                OR LOWER(u.full_name) LIKE LOWER(:word)
                OR LOWER(u.email) LIKE LOWER(:word) 
                OR LOWER(u.pseudo) LIKE LOWER(:word)
                OR LOWER(u.role) LIKE LOWER(:word)
            )
             ORDER BY v.end_at ASC",
                [
                    "owner" => $owner,
            ...
            $list_items = [];
            foreach ($rows as $item) {
                // Ajout de données supplémentaires facilités l'implémentation des cards html.
                $list_items[] = [
                    "title" =>       (string)$item["title"],
                    "description" =>    (string)$item["description"],
                    "duration_days" => (int)$item["duration_days"],
    ...
    // Récupére les annonces fermées qui sont détenue par l'utilisateur courant.
    public static function get_closed_unsold_items_by_owner(int $owner): array
    {
        try {
            $pdo = self::execute(
                "SELECT Distinct v.*, ip.picture_path
             FROM v_items_status v
             LEFT JOIN item_pictures ip ON ip.item = v.id AND ip.priority = 1 -- Simplification si priority 1 est toujours la principale
             WHERE v.owner = :owner
               AND v.end_at <= :now
               -- AND v.has_bids = 0-- 
    ...
    // Récupére les annonces fermées qui sont détenue par l'utilisateur courant pour le service JS.
    public static function get_closed_unsold_items_by_owner_service(int $owner, string $word): array
    {
        try {
            $pdo = self::execute(
                "SELECT Distinct v.*, ip.picture_path
    ...
    // Liste d'item vendue par un utilisateur.
    ...
    // Liste d'item vendue par un utilisateur.
    public static function get_sold_items_by_owner_service(int $owner, string $word): array
    {
        try {
            $pdo = self::execute(
                "SELECT DISTINCT v.*, ip.picture_path
             FROM v_items_status v
             JOIN users u ON v.owner = u.id        
             LEFT JOIN item_pictures ip ON ip.item = v.id AND ip.priority = 1 -- Les consignes utilisent la priorité 1 pour la photo principale
             WHERE v.owner = :owner
               AND (
                    v.buy_now_reached = 1
                    OR (v.end_at <= :now AND v.has_bids = 1)
   ...
   ========================================================= */
    ...
    public static function other_available_items($user_id){
        try {
            $pdo = self::execute("
               SELECT DISTINCT s.*
                    FROM v_items_status s
                    WHERE s.owner != :user_id
                    AND s.id NOT IN (
                        SELECT b.item FROM bids b
                        WHERE b.owner = :user_id2
                    )
                    AND s.end_at > :now
                    AND s.buy_now_reached = 0
                    AND NOT (s.is_direct_sale = 1 AND s.has_bids = 1)
    ...
    public static function set_item_categories(int $item_id, array $category_ids): bool {
        try {
            self::execute("DELETE FROM item_categories WHERE item = :item_id",
                ["item_id" => $item_id]);
            foreach ($category_ids as $cat_id) {
                self::execute(
                    "INSERT INTO item_categories (item, category) VALUES (:item_id, :cat_id)",
                    ["item_id" => $item_id, "cat_id" => (int)$cat_id]
                );
            }
            return true;
        } catch (PDOException $e) {
    ...
    public static function get_category_ids_for_item(int $item_id): array {
        try {
            $pdo = self::execute(
                "SELECT category FROM item_categories WHERE item = :id",
                ["id" => $item_id]
            );
            if (!$pdo) return [];
            $rows = $pdo->fetchAll(PDO::FETCH_COLUMN);
    ...
    // Obtenir les items disponibles auquelles l'utilisateur ne participent pas.
    public static function other_available_items_service(int $user_id,string $word){
        try {
            $pdo = self::execute("
               SELECT DISTINCT s.*
                    FROM v_items_status s
                    ...
                    AND NOT (s.is_direct_sale = 1 AND s.has_bids = 1)
                    AND (
                        LOWER(s.title) LIKE LOWER(:word)
                        OR LOWER(s.description) LIKE LOWER(:word)
                        OR LOWER(u.full_name) LIKE LOWER(:word)
                        OR LOWER(u.email) LIKE LOWER(:word) 
                        OR LOWER(u.pseudo) LIKE LOWER(:word)
                        OR LOWER(u.role) LIKE LOWER(:word)
    ...
    public function get_highest_bidder():bool|Bids{
        try {
            $pdo = self::execute("SELECT b.* FROM bids b
                                ...
                                AND (
                                    i.starting_bid IS NOT NULL
                                    AND i.starting_bid > 0   
                                    AND b.amount >= i.starting_bid
                                    OR 
                                    i.buy_now_price IS NOT NULL
                                    AND b.amount >= i.buy_now_price
                                )
                                ORDER BY b.amount DESC, b.created_at ASC
                                LIMIT 1
...
//                ));
    ...
    /* =========================================================
        Sales
    ========================================================= */
    // Le gagnant  le plus fréquent mes acheteurs.
    public static function get_recurring_winning_item_owned_by(int $owner_id):int{
        try {
            $pdo = self::execute("SELECT 
                                    b.owner AS best_buyer_id, 
                                    COUNT(b.item) AS total_wins
                                FROM v_items_status v
                                JOIN bids b 
                                    ON b.item = v.id 
            ...
            $recurring_winner_id = (int)$row['best_buyer_id'];
            return $recurring_winner_id;
        } catch (PDOException $e) {
            return false;
        }
    }
    ...
    /* =========================================================
        Purchases
    ========================================================= */
    // Donne la liste des items remportée par un utilisateur.
    public static function get_items_win_by(int $user_id): bool|array{
        try {
            $pdo = self::execute("SELECT Distinct i.* 
                                      FROM items i 
                                      JOIN bids b ON b.item = i.id
            ...
            $list_items_win = array();
            if(!empty($list_items)){
                foreach ($list_items as $item){
                    $bid = $item->has_a_winner();
                    if($bid instanceof Bids){
                        if($bid->get_owner_id() === $user_id){
                            $list_items_win[]= $item;
                        }
                    }
                }
    ...
    // Le vendeur a qui j'ai acheté le plus d'items.
    public static function who_i_m_bought_from_the_most(int $user_id):int{

        try {
            $pdo = self::execute("SELECT 
                                    v.owner AS seller_id,
                                    COUNT(v.id) AS total_items_bought
                                FROM v_items_status v
                                JOIN bids b 
                                    ON b.item = v.id 
                                    AND b.amount = v.max_bid 
                                WHERE b.owner = :user_id
                                  AND (
                                       v.buy_now_reached = 1
                                       OR (v.end_at <= :now AND v.has_bids = 1) -- (Ou :now en PHP)
                                  )
                                GROUP BY v.owner
    ...
    public function to_json(): array {
        // Récupération de l'ID du meilleur enchérisseur pour la logique d'affichage des badges.
        $highest_bidder = $this->get_highest_bidder();
        $highest_bidder_id = ($highest_bidder instanceof Bids) ? $highest_bidder->get_owner_id() : null;
        ...
        $data = [
            ...
            'owner_pseudo'        => $this->get_owner_pseudo(),

            // Dates et temps
            'created_at'          => $this->get_created_at()->format('Y-m-d H:i:s'),
            'end_at'              => $this->get_end_at() ? $this->get_end_at()->format('Y-m-d H:i:s') : null,
            'duration_days'       => $this->get_duration_days(),
            'time_has_passed'     => $this->time_has_passed(),
            'time_elapsed_string' => $this->time_elapsed_string(),

            // Prix et enchères
            'buy_now_price'       => $this->get_buy_now_price(),
            'starting_bid'        => $this->get_starting_bid(),
            'max_bid'             => $this->get_max_bid(),
            'bid_count'           => $this->get_bid_count(),
            'highest_bidder_id'   => $highest_bidder_id,

            // Booléens d'états (règles métiers & Vues)
            'has_auction'         => $this->has_auction(),
            'has_buy_now'         => $this->has_buy_now(),
            'has_bids'            => $this->is_has_bids(),
            'is_direct_sale'      => $this->is_direct_sale(),
            'is_auction'          => $this->is_auction(),
            'buy_now_reached'     => $this->get_buy_now_reached(),
            'not_purchased_direct_sale' => $this->is_not_purchased_direct_sale(),

            // Images
            'pictures'            => $this->get_picture_path_item() ?: []
        ];
