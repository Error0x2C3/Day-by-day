        ...
        <div class="colonne-droite">   <!--- classe parent qui gére la colonne droite. --->
            ...
            <?php if($item->get_owner_id() == $user->get_id() && isset($item)): ?>
                <div class="manage-your-item card">
                    ...
                    <?php
                    $btn_back = Navigation::sanitize("item/open_item_view/".$item->get_id());
                    ?>
                    <a href= "item/edit_item_view/<?=$item->get_id()?>/<?=$btn_back?>" class="btn edit-item-details-btn w-100">
                        <i class="bi bi-pen me-2"></i>Edit item Details
                    </a>
                    <?php
                    $btn_back = Navigation::sanitize("item/open_item_view/".$item->get_id());
                    ?>
                    <a href="item/manage_images_view/<?=$item->get_id()?>/<?=$btn_back?>" class="btn manage-images-btn w-100">
                        <i class="bi bi-image me-2"></i>Manage Images
                    </a>
                    <?php
                    $btn_back = Navigation::sanitize("item/open_item_view/".$item->get_id());
                    ?>
                    <a href="item/delete/<?php $item->get_id() ?>/<?=$btn_back?>" class="btn delete-item-btn w-100">
